...
# =============================================================================
# CTC Beam Search Top-K Decoder
# =============================================================================
# 在标准 CTC greedy decoder 的基础上增加 beam search，
# 从模型输出的完整 (T, C) 概率矩阵中提取 top-K 候选文本序列。
# 用于对低置信度 OCR 结果做语义重排序，不依赖具体混淆对的硬编码规则。
# =============================================================================

class CTCLabelDecodeTopK:
    """
    CTC beam search for top-K text candidates.

    与标准 CTCLabelDecode(greedy) 不同，此类使用完整的 (T, C) softmax 概率矩阵
    进行 beam search，返回多个候选文本及其归一化分数。

    用途：当 greedy result 置信度低时，用 top-K 候选 + 语义上下文重新选择。
    """

    def __init__(self, character, beam_size=10, blank_idx=0):
        """
        character: numpy array of characters (vocabulary), index 0 = blank
        beam_size: beam width during search
    ...
    def decode(self, preds_tc, top_k=5):
        """
        对单张图片的 softmax 输出做 CTC beam search。

        preds_tc: (T, C) numpy array, softmax probabilities
        top_k: 返回前 K 个候选

        Returns: [(text, score), ...], score 为指数空间的归一化概率近似值
        """
        return ctc_beam_search(preds_tc, self.character, self.beam_size, self.blank_idx)[:top_k]


def ctc_beam_search(probs, vocabulary, beam_size=10, blank_idx=0):
    """
    Standard CTC beam search decoder.

    Implemented following Graves et al. with separate blank/non-blank
    probability tracking for correct CTC merge rules.

    probs: (T, C) numpy array — softmax probabilities
    vocabulary: numpy array of characters indexed by C dimension
    beam_size: beam width
    blank_idx: index of CTC blank token

    Returns: [(text, score), ...] sorted by score descending
    """
    import numpy as np
    import math
    ...
    # Each beam element: (prefix_tuple, log_prob_blank, log_prob_non_blank)
    # prob_blank: prefix ends with blank label
    # prob_non_blank: prefix ends with non-blank label
    beam = [((), 0.0, float('-inf'))]

    for t in range(T):
        next_beam = {}  # key: (prefix, last_char) → (log_pb, log_pnb)

        for prefix, log_pb, log_pnb in beam:
            for c in range(C):
                log_p = log_probs[t, c]

                if c == blank_idx:
                    # ── Extend with blank ──
                    new_key = (prefix, blank_idx)
                    new_log_pb = _log_sum_exp(log_pb + log_p, log_pnb + log_p)
                    if new_key in next_beam:
                        existing = next_beam[new_key]
                        next_beam[new_key] = (_log_sum_exp(existing[0], new_log_pb), existing[1])
                    else:
                        next_beam[new_key] = (new_log_pb, float('-inf'))
                else:
                    if len(prefix) > 0 and prefix[-1] == c:
                        # ── Same character repeated → CTC merge ──
                        new_key = (prefix, c)
                        new_log_pnb = log_pnb + log_p
                        if new_key in next_beam:
                            existing = next_beam[new_key]
                            next_beam[new_key] = (existing[0], _log_sum_exp(existing[1], new_log_pnb))
                        else:
                            next_beam[new_key] = (float('-inf'), new_log_pnb)
                    else:
                        # ── New character ──
                        new_prefix = prefix + (c,)
                        new_key = (new_prefix, c)
                        new_log_pnb = _log_sum_exp(log_pb + log_p, log_pnb + log_p)
                        if new_key in next_beam:
                            existing = next_beam[new_key]
                            next_beam[new_key] = (existing[0], _log_sum_exp(existing[1], new_log_pnb))
                        else:
        ...
        # ── Prune beam to top beam_size by total score ──
        scored = []
        for (prefix, last), (log_pb, log_pnb) in next_beam.items():
            total = _log_sum_exp(log_pb, log_pnb)
            scored.append((prefix, log_pb, log_pnb, total))
        scored.sort(key=lambda x: x[3], reverse=True)
        beam = [(p, b, n) for p, b, n, _ in scored[:beam_size]]

    # ── Collect and sort final results ──
    results = []
    for prefix, log_pb, log_pnb in beam:
        total_log = _log_sum_exp(log_pb, log_pnb)
        text = ''.join(str(vocabulary[idx]) for idx in prefix)
        approx_score = float(np.exp(total_log / max(1, T)))
        results.append((text, approx_score))
...
def _log_sum_exp(a, b):
    """log(exp(a) + exp(b)) with -inf safety"""
    import math
    if a == float('-inf'):
        return b
    if b == float('-inf'):
