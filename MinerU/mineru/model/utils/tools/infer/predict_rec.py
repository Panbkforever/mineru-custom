...
class TextRecognizer(BaseOCRV20):
    def __init__(self, args, **kwargs):
        ...
        # [新增] top-K 候选支持
        # return_preds=True 时，__call__ 会将 preds softmax 保存到 self._table_preds，
        # 之后可以通过 get_top_k_candidates() 获取任意图像的 CTC beam search 候选
        self.return_preds = False
        self._table_preds = None  # list of (T, C) numpy arrays, 与 img_list 顺序对齐
        ...
        with tqdm(total=img_num, desc=tqdm_desc, disable=not tqdm_enable) as pbar:
            ...
            for beg_img_no in range(0, img_num, batch_num):
                ...
                with torch.no_grad():
                    ...
                    # [新增] 保存 preds 供外部 top-K 查询
                    # 仅当 return_preds=True 时启用（由 batch_analyze.py 在表格 OCR 步骤设置）
                    # self._table_preds 按 img_list 的原始输入顺序存储，
                    # 因为 indices 映射将 batch 顺序转回了原始顺序
                    if self.return_preds:
                        if self._table_preds is None:
                            self._table_preds = [None] * img_num
                        batch_count = len(rec_result)
                        for rno in range(batch_count):
                            orig_idx = indices[beg_img_no + rno]
                            self._table_preds[orig_idx] = preds[rno].cpu().numpy()
    ...
    # [新增] top-K 候选方法
    def get_top_k_candidates(self, global_idx, top_k=5, beam_size=10):
        """
        获取第 global_idx 个输入图像的 CTC beam search top-K 识别候选。

        前置条件：__call__ 必须在 return_preds=True 下被调用过，
                  且 self._table_preds 已被填充。

        参数：
            global_idx: 在 img_list 中的原始索引（0-based）
            top_k:      返回的候选数量
            beam_size:  beam search 宽度

        返回：
            [(text, score), ...] — 候选文本及其归一化分数列表
            如果没有可用 preds 则返回空列表
        """
        if self._table_preds is None or global_idx >= len(self._table_preds):
            return []
        preds_tc = self._table_preds[global_idx]
        if preds_tc is None:
            return []
        from ..pytorchocr.postprocess.rec_postprocess import CTCLabelDecodeTopK
        decoder = CTCLabelDecodeTopK(
            character=self.postprocess_op.character,
            beam_size=beam_size,
            blank_idx=0,
        )
        return decoder.decode(preds_tc, top_k=top_k)
