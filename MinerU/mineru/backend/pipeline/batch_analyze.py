    ...
    def __call__(self, images_with_extra_info: list) -> list:
        ...
        if self.table_enable:
            ...
            for _lang, rec_img_list in rec_img_lang_group.items():
                ...
                # [新增] 启用 TextRecognizer 的 preds 保存，以便后续 top-K beam search
                ocr_engine.text_recognizer.return_preds = True
                ocr_engine.text_recognizer._table_preds = None
                ocr_res_list = ocr_engine.ocr(cropped_img_list, det=False, tqdm_enable=True, tqdm_desc=f"Table-ocr rec {_lang}")[0]
                # [新增] 关闭 preds 保存（避免影响后续非表格 OCR）
                ocr_engine.text_recognizer.return_preds = False
                # 按照 table_id 将识别结果进行回填
                for img_idx, (img_dict, ocr_res) in enumerate(zip(rec_img_list, ocr_res_list)):
                    ocr_text = self._normalize_table_ocr_rec_text(ocr_res[0])
                    ocr_score = ocr_res[1]
                    # [新增] 对低置信度单元格获取 top-K 候选
                    candidates = []
                    if ocr_score < 0.7:
                        try:
                            candidates = ocr_engine.text_recognizer.get_top_k_candidates(
                                img_idx, top_k=5
                            )
                        except Exception:
                            candidates = []
                    # ocr_result_item: [dt_box, text, confidence, candidates]
                    ocr_result_item = [img_dict["dt_box"], html.escape(ocr_text), ocr_score, candidates]
            ...
            # [新增] 表格 OCR 上下文感知修正
            # 利用 OCR 模型输出的 top-K 候选 + 行级语义打分，
            # 对低置信度字符做消歧修正。不依赖具体混淆对（I→1 等）的硬编码规则。
            # 在 OCR 识别完成后、表格结构模型运行前执行，
            # 此时数据包含坐标（可重建行分组）、文本和候选信息。
            from post_table.context_correct import apply_table_context_correction
            apply_table_context_correction(table_res_list_all_page)
