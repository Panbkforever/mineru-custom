...
def test_family_member_identity_rows_merge_into_existing_physical_slots():
    """模型多返回 family members 时，应按 drawing/pin_count 收敛回强槽。"""

    strong_summary = catalog_table(
        0,
        "器件信息",
        ["DEVICE", "PACKAGE", "DRAWING", "PINS"],
        [
            ["DEVICE", "PACKAGE", "DRAWING", "PINS"],
            ["CC430F6147IRGC", "VQFN", "RGC", "64"],
            ["CC430F5147IRGZ", "VQFN", "RGZ", "48"],
        ],
    )
    ...
    target_64 = target_table(
        2,
        "Table 4-1. CC430F614x Terminal Functions",
        ["PIN NO", "PIN NAME", "TYPE"],
    )
    target_48 = target_table(
        3,
        "Table 4-2. CC430F514x and CC430F512x Terminal Functions",
        ["PIN NO", "PIN NAME", "TYPE"],
    )
    ...
    def classifier(table, source_name, target_tables):
        ...
        return {"is_package_summary": False}

    result = resolve_document_package_catalog(
        all_tables=[strong_summary, family_members],
        target_tables=[target_64, target_48],
        multi_package_plans={
            2: MultiPackagePlan(False, "single_package"),
            3: MultiPackagePlan(False, "single_package"),
        },
        source_name="CC430F514x、CC430F614x_64_48",
        use_semantic_classifier=True,
        classifier=classifier,
    )
    ...
    assert len(result.entries) == 2
    by_drawing = {
        entry.package_drawing: entry
        for entry in result.entries
    }
    assert set(by_drawing) == {"RGC", "RGZ"}
    assert by_drawing["RGC"].pin_count == "64"
    assert by_drawing["RGZ"].pin_count == "48"
    assert {"CC430F6147", "CC430F6145", "CC430F6143"}.issubset(
        set(by_drawing["RGC"].identity_aliases)
    )
    assert {"CC430F5145", "CC430F5123"}.issubset(
        set(by_drawing["RGZ"].identity_aliases)
    )
    assert result.assignment_for(2, 0).package_key == "slot:0"
    assert result.assignment_for(3, 0).package_key == "slot:1"
