...
6. 封装类别：从总述表读取完整 device-pkg 关系，再结合已确认引脚表的
   表题/表头确定文档级类别数量和完整 pkg 名称。
7. 封装绑定：类别冻结后，沿用既有证据规则绑定每张引脚表；类别判断不能
   直接返回或修改表格绑定。
8. 行提取：单封装和多封装走各自独立逻辑，完整读取已经绑定的数据行。
9. pkg 内去重：只按完全相同的 pin_no、pin_name、type 合并重复记录，并汇总描述。
10. 结果整理：按固定槽位分组；pkg 使用完整封装名，未知时按 a/b/c 回退。
...
* ``package_catalog_resolver.py`` 是唯一的真实 pkg 判断模块。它先从全文表格
  定位封装总述候选并读取完整 device-pkg 关系；再以一次文档级类别调用，
  综合这些关系和已确认引脚表的完整表题/表头，确定类别数量和完整 pkg 名称；
  最后才使用既有绑定规则处理每张表，类别模型本身不能返回表格绑定。
...
* 器件型号只参与类别判断和槽位关联，不能写入 pkg。第三阶段确认的 pkg
  保留源关系中的完整封装类型、drawing/code 和 pin/ball 数量；名称不明确
  时按固定槽位顺序使用 a、b、c……，但槽位数量保持不变。
* 一个 pkg 只能是一个名称字符串，禁止使用 ``|`` 拼接多个名称；真实名称
  必须来自总述表源关系，模型不得缩写、改写或编造。
