...
表格中召回总述候选，再由独立模型协议只返回表格角色、表头行和列角色。模型
不再生成 pkg 名称；代码按照原始表格逐行读取 ``package_identity`` 建立目录，
然后使用表题、当前章节、表头和已确认的多封装绑定将引脚表归入对应封装。

身份总述表和包装信息表严格分工：

- ``identity_summary`` 表的 ``package_identity`` 列可以创建 pkg。
- ``packaging_metadata`` 表只能通过 ``orderable_sku`` 前缀匹配已有 pkg，
  补充 ``package_type``、``package_drawing`` 和 ``pin_count``。
- SC-70、VSSOP、QFN 等封装类型和 DCK、DGK、RGV 等 Drawing 不能单独创建
  pkg；订购型号也不能创建 pkg。
- 模型会同时看到已选引脚表的标题、章节和表头，但不会看到这些表的数据行，
  这些上下文只帮助它识别总述表中的身份列。
...
## 2026-07-28：封装数量优先与物理封装名称

本节规则取代上面“器件身份直接作为 pkg”和“每张未匹配表独立建包”的旧规则。

- `extract/package_catalog_resolver.py`
  - 将器件型号和物理封装名彻底分离：`package_identity` 只保存到内部
    `identity_name`，用于把表题中的 INA290 等型号关联到对应槽位；公开
    `pkg` 只读取 `package_type`，例如 SC-70、VSSOP、QFN。
  - 先根据身份总述表逐行建立物理引脚映射槽位；没有身份总述时，允许用
    包装表中的 package_type + drawing + pin_count 唯一组合，或一张严格
    确认的多封装表确定槽位数量。
  - 槽位统一冻结为 `slot:0`、`slot:1`……。冻结后所有表只能绑定已有槽位，
    禁止再生成 `unresolved:<table_id>`，避免把表格数量误当成封装数量。
  - 多张表都没有封装证据时，整篇文档只建立一个槽位；多封装计划兜底时
    只选槽位数最多的一张锚点表，不能累计多个表中的重复封装标签。
  - package_type 缺失时按固定槽位顺序输出 a、b、c……；回退名称只补名称，
    不增删槽位。相同公开名称的两个独立槽位仍保持分离。

- `extract/pin_package_extractor.py`
  - 行提取开始前先为全部冻结槽位建立外层输出桶。某个槽位尚未关联到引脚表
    时仍输出该槽位和空 `group_list`，确保最终 package 数量不被数据关联结果
    意外改变。
  - 调试信息分别输出 `identity_name`、`package_type`、drawing 和 pin_count，
    便于区分“型号关联证据”和“公开物理封装名”。

- `extract/semantic_classifier.py`
  - 模型仍只返回总述表角色、表头行和列角色，不返回单元格值。
  - 提示词明确 package_identity 是内部器件型号，package_type 才是公开物理
    封装名；Drawing 只用于区分封装变体。

- 通用测试
  - 验证 INA290/INA2290/INA4290 只用于内部关联，最终 pkg 为
    SC-70/VSSOP/QFN。
  - 验证多张未匹配表只落入一个固定槽位，不再生成多个空 pkg。
  - 验证同为 QFN 的两个独立型号槽位不会被名称相同而合并。
  - 验证已确认的槽位即使暂时没有引脚记录，也保留在最终输出中。
